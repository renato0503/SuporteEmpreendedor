    ...
    <div class="p-8 lg:p-12 space-y-8 max-w-[1600px] mx-auto animate-fade-in">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <button id="btn-new-lead" class="px-6 py-3 bg-[#125133] text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-[#125133]/20 hover:scale-105 transition-transform flex items-center gap-2">
...
<td class="px-8 py-6">
             <select class="status-select px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${config.color} bg-transparent cursor-pointer" data-lead-id="${lead.id}" data-current-status="${lead.status}">
               <option value="novo" ${lead.status === 'novo' ? 'selected' : ''}>Novo</option>
               <option value="primeiro_contato" ${lead.status === 'primeiro_contato' ? 'selected' : ''}>1º Contato</option>
               <option value="em_negociacao" ${lead.status === 'em_negociacao' ? 'selected' : ''}>Negociação</option>
               <option value="aguardando_pagamento" ${lead.status === 'aguardando_pagamento' ? 'selected' : ''}>Aguardando Pagto</option>
               <option value="em_execucao" ${lead.status === 'em_execucao' ? 'selected' : ''}>Em Execução</option>
...
// Attach row events
    ...
    document.querySelectorAll('.status-select').forEach(select => {
      select.addEventListener('change', async (e) => {
        ...
        const newStatus = el.value as LeadStatus;
        if (leadId) {
  ...
  $('#btn-new-lead')?.addEventListener('click', () => {
    openLeadFormModal({}, () => {
      // Success callback - the snapshot listener will handle update
    });
  });
